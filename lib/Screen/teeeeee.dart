...
import 'package:eshop_multivendor/Helper/utility_helper.dart';
    ...
    if (_isNetworkAvail) {
      try {
        ...
        if (!error) {
          ...
          print(response.body);
          print(getdata["latitude"].toString());
          print(getdata["latitude"].toString());
          SharedPreferences preferences = await SharedPreferences.getInstance();
          await preferences.setString(
              sellerlatitude, getdata["latitude"].toString());
          await preferences.setString(
              sellerlongitude, getdata["longitude"].toString());
          print("===============");
  ...
  Future<void> _getAddress() async {
    ...
    if (_isNetworkAvail) {
      try {
        ...
        var lat1 = addressList[0].latitude;
        var lon1 = addressList[0].longitude;
        var lat2 = await preferences.getString(sellerlatitude);
        var lon2 = await preferences.getString(sellerlongitude);
        // var distanceKM =
        //     UtilityHelper.calculateDistance(lat1, lon1, lat2, lon2);
        var distanceKM = 5.0555;
        Response responseDeliveryCharge =
            await get(getDeliveryChargeApi, headers: headers)
                .timeout(Duration(seconds: timeOut));
        print(responseDeliveryCharge.body);

        var getDeliveryCharge = jsonDecode(responseDeliveryCharge.body);

        for (var i = 0; i < getDeliveryCharge["Charge"].length; i++) {
          print(i.toString());
          double distance = double.parse(distanceKM.toString());
          double deliveryCharge = double.parse(
              getDeliveryCharge["Charge"][i]["delivery_charge"].toString());
          print("==================MMM");
          print(distance.toDouble());
          print(deliveryCharge.toDouble());
          print("==================MMM");
          // if (double.parse(distanceKM.toStringAsFixed(0)) <=
          //     double.parse(getDeliveryCharge["Charge"][0]["max_km"]
          //         .toStringAsFixed(0))) {
          //   print("====Done=======");
          //   print(getDeliveryCharge["Charge"][i]["delivery_charge"].toString());
          //   print("====done=======");
          // }
        }
        print("====distanceKM=======");
