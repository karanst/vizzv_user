...
import 'package:eshop_multivendor/Screen/my_favorite_seller/my_favorites_seller.dart';
...
class StateProfile extends State<MyProfile> with TickerProviderStateMixin {
  ...
  Future<void> setUpdateUser(String userID,
      [oldPwd, newPwd, username, userEmail, altNo]) async {
    var apiBaseHelper = ApiBaseHelper();
    var data = {USER_ID: userID, "alternate_number": "$altNo"};
  ...
  _getDrawer() {
    return ListView(
      ...
      children: <Widget>[
        ...
        CUR_USERID == "" || CUR_USERID == null
            ? Container()
            : _getDrawerItem("My Favourites",
                imagePath + "desel_fav.svg"),
        CUR_USERID == "" || CUR_USERID == null
            ? Container()
            : _getDrawerItem(
                "My Favorites Seller", 'assets/images/pro_myorder.svg'),
...
/*  _getDivider() {
    ...
    return Card(
      ...
      child: ListTile(
        ...
        onTap: () {
          ...
          else if (title == "My Favorites Seller") {
            Navigator.push(context,
                MaterialPageRoute(builder: (context) => MyFavoritesSeller()));
          }
        },
      ),
    );
  ...
  Widget setAlternativeNo() => Padding(
        padding: const EdgeInsets.symmetric(vertical: 8.0, horizontal: 15.0),
        child: Container(
          padding:
              EdgeInsets.only(bottom: MediaQuery.of(context).viewInsets.bottom),
          ...
          child: Padding(
            ...
            child: TextFormField(
              decoration: InputDecoration(
                  hintText: 'Alternative Mobile No.', labelText: "Mobile No."),
            ),
          ),
        ),
      );
  ...
  Future<bool> validateAndSave(GlobalKey<FormState> key) async {
    ...
    if (form.validate()) {
      if (key == _changePwdKey) {
        ...
        setUpdateUser(CUR_USERID!, "", "", nameController.text,
            emailController.text, altNumberController.text);
      }
      return true;
    }
    return false;
