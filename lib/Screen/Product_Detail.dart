...
class ProductDetail extends StatefulWidget {
  ...
  ProductDetail(
      {Key? key, this.model, this.secPos, this.index, this.list, this.sellerId})
      : super(key: key);
...
*/
  ...
  _price(pos, dynamic from, String timeData) {
  ...
  _packingCharge() {
    String? packingCharge = widget.model!.packing_charge;

    return packingCharge != ""
        ? Padding(
            padding: EdgeInsets.symmetric(horizontal: 20),
            child: ListTile(
              trailing: Text("$CUR_CURRENCY ${packingCharge.toString()}"),
              dense: true,
              title: Text(
      ...
      if (CUR_USERID != null) {
        try {
          ...
          if (!error) {
            ...
            context.read<UserProvider>().setAmount(getdata['data']['overall_amount']);

            context.read<UserProvider>().setStoreName(getdata['cart'][0]['product_details'][0]['store_name']);

            context.read<UserProvider>().setSellerId(getdata['cart'][0]['product_details'][0]['seller_id']);

            context.read<UserProvider>().setSellerProfile(getdata['cart'][0]['product_details'][0]['seller_profile']);
  ...
  _showContent() {
    ...
    if (widget.model!.breakfast_start_product_time != null &&
        widget.model!.breakfast_end_product_time != null) {
      List hM = widget.model!.breakfast_start_product_time
          .toString()
          .split(":")
          .toList();
      List hM1 = widget.model!.breakfast_end_product_time
          .toString()
          .split(":")
          .toList();
      // print("${time.hour}:${time.minute}");
      DateTime t = DateTime(
          time.year, time.month, time.day, int.parse(hM[0]), int.parse(hM[1]));
      DateTime m = DateTime(time.year, time.month, time.day, int.parse(hM1[0]),
          int.parse(hM1[1]));
      if (time.isAfter(t) && time.isBefore(m)) {
        ...
        if (time.isAfter(t)) {
          ...
          timeData = widget.model!.breakfast_start_product_time.toString();
        }
      }
    } else {}
    if (timeData != "Yes" && timeData == "") {
      if (widget.model!.lunch_start_product_time != null &&
          widget.model!.lunch_end_product_time != null) {
        List hM = widget.model!.lunch_start_product_time
            .toString()
            .split(":")
            .toList();
        List hM1 =
            widget.model!.lunch_end_product_time.toString().split(":").toList();
        //  print("${time.hour}:${time.minute}");
        ...
        if (time.isAfter(t) && time.isBefore(m)) {
          ...
          if (time.isAfter(t)) {
            ...
            timeData = widget.model!.lunch_start_product_time.toString();
          }
        }
      } else {}
    }
    if (timeData != "Yes" && timeData == "") {
      if (widget.model!.dinner_start_product_time != null &&
          widget.model!.dinner_end_product_time != null) {
        List hM = widget.model!.dinner_start_product_time
            .toString()
            .split(":")
            .toList();
        List hM1 = widget.model!.dinner_end_product_time
            .toString()
            .split(":")
            .toList();
        //  print("${time.hour}:${time.minute}");
        ...
        if (time.isAfter(t) && time.isBefore(m)) {
          ...
          if (time.isAfter(t)) {
            ...
            timeData = widget.model!.dinner_start_product_time.toString();
          }
        }
      } else {}
    }
    ...
    if (widget.model!.breakfast_start_product_time == null &&
        widget.model!.breakfast_end_product_time == null &&
        widget.model!.lunch_start_product_time == null &&
        widget.model!.lunch_end_product_time == null &&
        widget.model!.dinner_start_product_time == null &&
        widget.model!.dinner_end_product_time == null) {
      timeData = "No";
    } else {
      if (widget.model!.dinner_end_product_time != null &&
          widget.model!.dinner_start_product_time != null) {
        List hM = widget.model!.dinner_start_product_time
            .toString()
            .split(":")
        ...
        if (time.isAfter(t)) {
          if (widget.model!.breakfast_start_product_time != null) {
            timeData = widget.model!.breakfast_start_product_time.toString() +
                ",Tomorrow";
          } else if (widget.model!.lunch_start_product_time != null) {
            timeData =
                widget.model!.lunch_start_product_time.toString() + ",Tomorrow";
          } else if (widget.model!.dinner_start_product_time != null) {
            timeData = widget.model!.dinner_start_product_time.toString() +
                ",Tomorrow";
          }
        }
      } else if (widget.model!.lunch_end_product_time != null) {
        List hM =
            widget.model!.lunch_end_product_time.toString().split(":").toList();
        DateTime t = DateTime(time.year, time.month, time.day, int.parse(hM[0]),
            int.parse(hM[1]));
      ...
      } else if (widget.model!.breakfast_end_product_time != null) {
        List hM = widget.model!.breakfast_end_product_time
            .toString()
            .split(":")
            .toList();
        DateTime t = DateTime(time.year, time.month, time.day, int.parse(hM[0]),
    ...
    return Scaffold(
     ...
     child:  AppBar(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      (widget.model!.rating! == "0" ||
                          ...
                          ? Container()
                          : _rate(),
                      _price(widget.model!.selVarient, true, timeData),
      ...
      bottomSheet:  widget.model!.availability == "1" || widget.model!.stockType == "null"
          ? int.parse(qtyController.text) > 0
          ? Container(
        height: 45,
        decoration: BoxDecoration(
    ...
    if (_isNetworkAvail) {
      try {
        if (notificationisloadmore) {
          ...
          print(parameter);
          if (CUR_USERID != null) parameter[USER_ID] = CUR_USERID.toString();

          Response response =
              await post(moreProductApi, headers: headers, body: parameter)
                  .timeout(Duration(seconds: timeOut));

          var getdata = json.decode(response.body);
          // log("${StackTrace.current} ${response.body}");
          print("testdata" + getdata.toString());
          bool error = getdata["error"];
          // String msg = getdata["message"];
  ...
  _shortDesc() {
    print(widget.model!.shortDescription.toString().replaceAll("\\r", ""));
    return widget.model!.shortDescription != null &&
            widget.model!.shortDescription!.isNotEmpty
        ? Padding(
            padding: const EdgeInsetsDirectional.only(
                start: 8, end: 8, bottom: 5),
            child: Text(
              parseString(widget.model!.shortDescription!.toString())
                  .replaceAll("\\r", "")
                  .replaceAll("\\n", "\n")
                  .replaceAll("/", " ")
                  .replaceAll("\\", ""),
